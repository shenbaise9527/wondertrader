...
public:
    ...
    static inline std::string rawMonthCodeToStdCode(const char* code,
        ...
        if (isComm) {
            ...
            if (strlen(s) == 4) {
                ...
                // TODO: 针对CZCE的合约代码，如果最后一段是3位数字，则需要补齐到4位,默认补齐为2.
                buffer[len] = '2';
                len += 1;
     ...
     *	这种主要针对非分月合约而言
     */
    static inline std::string rawFlatCodeToStdCode(const char* code,
                                                   const char* exchg,
                                                   const char* pid) {
        thread_local static char buffer[64] = {0};
    ...
    static inline std::string rawFutOptCodeToStdCode(const char* code,
        ...
        if (bMatch) {
            ...
            s.append(".");
            s.append(code, idx - 3);
            if (strcmp(exchg, "CZCE") == 0) s.append("2");
    ...
    static inline std::string stdFutOptCodeToRawCode(const char* stdCode) {
        ...
        if (strncmp(stdCode, "CFFEX", 5) == 0 ||
            strncmp(stdCode, "DCE", 3) == 0)
            StrUtil::replace(ret, ".", "-");
        else
            StrUtil::replace(ret, ".", "");
        return ret;
    ...
    static CodeInfo extractStdChnFutOptCode(const char* stdCode) {
        ...
        if (strcmp(codeInfo._exchg, "SHFE") == 0 ||
            ...
            std::string& s = ay[1];
            fmt::format_to(codeInfo._code, "{}{}{}{}",
                           s.substr(0, s.size() - 4), s.substr(s.size() - 3),
                           ay[2], ay[3]);
        } else {
            fmt::format_to(codeInfo._code, "{}-{}-{}", ay[1], ay[2], ay[3]);
        }
        ...
        if (isStdChnFutOptCode(stdCode)) {
            ...
            if (idx2 == std::string::npos) {
                ...
                if (extlen == 4 && '0' <= lastCh && lastCh <= '9') {
                    ...
                    const char* ruleTag =
                        (hotMgr != NULL) ? hotMgr->getRuleTag(ext) : "";
                    if (strlen(ruleTag) == 0)
